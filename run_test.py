...
def main():
    ...
    if torch.cuda.device_count() > 1:
        model.module.load_state_dict(torch.load(base_config.output_dir + '/pytorch_model.bin'))
    else:
        model.load_state_dict(torch.load(base_config.output_dir + '/pytorch_model.bin'))

    evaluate(model, test_batch, base_config, args.task, is_test=True)
