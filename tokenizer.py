...
class tokenizer:
    ...
    def encode_slice(self, sentences, p_id=0):
        ...
        for line_id, inputexample in enumerate(sentences):
            ...
            try:
                ...
                dataset.append(((title_tokens, title_mask, title_pos_ids, title_vm), 
                               (keyword_tokens, keyword_mask, keyword_pos_ids, keyword_vm), 
                               (summary_tokens, summary_mask, summary_pos_ids, summary_vm), 
                               label))
            except:
                print("Error line: ", line_id)
        return dataset
    ...
    def encode_with_knowledge_slice(self, sentences, p_id=0):
        ...
        for line_id, inputexample in enumerate(sentences):
            ...
            try:
                label = inputexample.label
                title_text = [CLS_TOKEN] + list(inputexample.title)
                keyword_text = [CLS_TOKEN] + list(inputexample.keyword)
                summary_text = [CLS_TOKEN] + list(inputexample.summary)
